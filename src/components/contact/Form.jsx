...
export default function Form() {
  ...
  const toastId = toast.loading("Send your message, please wait...")
    emailjs
      .send(process.env.NEXT_PUBLIC_SERVICE_ID, process.env.NEXT_PUBLIC_TEMPLATE_ID, params,
        ...
        limitRate: {
            throttle: 5000, // you cannot send more than 1 email per 5s using this API
        }
        })
      .then(
        () => {
  ...
  return (
    ...
    <>
        ...
        <motion.textarea variants={container} placeholder='message' {...register("message", {required: "This field is required!", 
        maxLength: {
            value: 1000,
        ...
        minLength: {
            value: 50,
            message: "Message should contain atleast than 50 characters."
        }})} 
            className='w-full p-2 rounded-md shadow-lg text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 custom-bg'
        />
        {
